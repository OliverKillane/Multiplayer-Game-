...
->Create Client object:     CommunicationClient();
->Set up connection:        SetupConnection(QString IP, QString commport, QColor commcolour, QString commname);

->Send data:                connect(this, SIGNAL(Sending function), client, SLOT(SendData));
                            emit SendClientData(GameData data);

->Recieve Data:             connect(client, SIGNAL(RecieveData), this, SLOT(Recieving function));
*/
...
void CommunicationClient::SetupConnection(QString IP, QString commport, QColor commcolour, QString commname){

    // Setup attributes.
    address = QHostAddress(IP);
    colour = commcolour;
    port = commport.toUInt();
    name = commname;

    // Start TCP socket to connect to address.
    socket->connectToHost(address, port);

    // Connect signal for data recieved to slot.
    connect(socket, SIGNAL(ReadyRead()), this, SLOT(ServerDataRecieved()));

    // Connect error signal to handler
    connect(socket, SIGNAL(error(QAbstractSocket::SocketError)),this, SLOT(CatchError(QAbstractSocket::SocketError)));

}

void CommunicationClient::ServerDataRecieved(){
    ...
    emit RecieveData(unserializeddata);

    return;
}

void CommunicationClient::SendData(GameData data){
    ...
    // Sending serialized data (as a QByteArray) to the server.
    socket->write(serializeddata);

    return;
}

void CommunicationClient::CatchError(QAbstractSocket::SocketError socketError){

    // Taking the error, converting it to a text (QString) prompt and emitting the signal.
    switch (socketError) {
        case QAbstractSocket::RemoteHostClosedError:
            break;
        case QAbstractSocket::HostNotFoundError:
            emit Error(QString("No server found, please check the IP address is correct."));
            break;
        case QAbstractSocket::ConnectionRefusedError:
            emit Error(QString("Server has refused connection - may be full."));
            break;
        default:
            emit Error(QString("An Error has occured, please check the input IP and try again."));
    }
